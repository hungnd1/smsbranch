...
class Pagination
{
    ...
    {   
        ...
        {
            $p = '<div class="results-inner"><div class="pagination clearfix"><ul class="COGPager">';
             
            // Nút prev và first
            if ($this->_config['current_page'] > 1)
            {
                $p .= '<li class="COGPagerFirst"><a href="'.$this->__link('1').'"><<</a></li>';
                $p .= '<li class="COGPagerPrevious"><a href="'.$this->__link($this->_config['current_page']-1).'"><</a></li>';
            }
            ...
            for ($i = 1; $i <= $this->_config['total_page']; $i++)
            {
                ...
                if ($this->_config['current_page'] == $i){
                    $p .= '<li class="COGPagerItem" ><span style="font-size:12px;"><strong>'.$i.'</strong></span></li>';
                }
                else{
                    $p .= '<li class="COGPagerItem"><a href="'.$this->__link($i).'">'.$i.'</a></li>';
                }
            }
            ...
            if ($this->_config['current_page'] < $this->_config['total_page'])
            {
                $p .= '<li class="COGPagerNext"><a href="'.$this->__link($this->_config['current_page'] + 1).'">></a></li>';
                $p .= '<li class="COGPagerLast"><a href="'.$this->__link($this->_config['total_page']).'">>></a></li>';
            }
