...
class Pages extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			...
			array('page_parent, page_status, page_order, cate_id', 'numerical', 'integerOnly'=>true),
			array('page_title, page_sublink, page_tag, page_titleen,page_image,page_redirect', 'length', 'max'=>255),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('page_id, page_title, page_order,page_redirect, page_content, page_sublink, page_tag, page_createdate,page_image, page_parent, page_status, page_titleen, page_contenten, cate_id', 'safe', 'on'=>'search'),
		);
	}
	...
	public function attributeLabels()
	{
		return array(
			...
			'page_contenten' => Yii::t('lang', 'Nội dung tiếng anh'),
                        'page_image' => Yii::t('lang', 'Ảnh đại diện'),
                        'page_redirect' => Yii::t('lang','Chuyển hướng'),
                        'cate_id'=>'Link đến danh mục'
    ...
    public function getPageByParentId($parent_id){
        $criteria= new CDbCriteria();
        $criteria->compare('page_parent', $parent_id);
        $criteria->order = 'page_order ASC';
