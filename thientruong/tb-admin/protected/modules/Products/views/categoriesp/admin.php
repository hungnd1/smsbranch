...
<?php $this->widget('bootstrap.widgets.TbGridView', array(
	...
	'columns'=>array(
		...
		array(
                    ...
                    'type'=>'raw',
                    'value'=>function($data){
                        return '<a href="'.YII::app()->createUrl("/Products/Categoriesp/admin",array("id"=>$data->cate_id)).'">'.$data->cate_title.'</a>';
                    },
		...
		array(
                ...
                array(
                    ...
                    'type'=>'raw',
                    'value'=>function($data){
                        return '<img width="200" src="'.Yii::app()->baseUrl.'/uploads/'.$data->cate_image.'" alt="'.$data->cate_title.'" />'; 
                    }
                ),
                array(
                    'name'=>'cate_images_1',
                    'type'=>'raw',
                    'value'=>function($data){
                        return '<img width="200" src="'.Yii::app()->baseUrl.'/uploads/'.$data->cate_images_1.'" alt="'.$data->cate_title.'" />'; 
                    }
                ),    
                array(
                    ...
                    'type'=>'raw',
                    'value'=>function($data){
                        if($data->categories_type!=0){
                            $result = Categoriesp::$type[$data->categories_type];
                            return $result;
                        }
                        else
                            return "";
