...
class ContactCategorieController extends Controller
{
    ...
    public function accessRules()
    {
        return array(
            ...
            array('allow', // allow authenticated user to perform 'create' and 'update' actions
                'actions' => array('create', 'update', 'DownloadTemplate', 'ImportDataExel', 'UpdateLineContactCategoty', 'LoadOptionMember', 'Birthday', 'AutoComplete', 'sendSms', 'MoveContact'),
                'users' => array('@'),
            ),
    ...
    public function actionAutoComplete($term)
    {
        ...
        $query = ContactCategorie::model()->findAll(
            $condition = ' category_name like :param1 ',
            $params = array(
                ':param1' => '%' . $term . '%',
            )
        );
        $list = array();
        foreach ($query as $q) {
            $data['value'] = $q['category_name'];
            $data['label'] = $q['category_name'];
    ...
    {
        if (isset($_POST['arr_member']) && isset($_POST['search']) && isset($_POST['id'])) {
            $result = 0;
            $query = ContactCategorie::model()->findAllByAttributes(array('category_name' => $_POST['search']));
            if (isset($query[0]['category_id'])) {
                for ($i = 0; $i < sizeof($_POST['arr_member']); $i++) {
                    $model = Contact::model()->findByPk($_POST['arr_member'][$i]);
                    $model->category_id = $query[0]['category_id'];
                    if ($model->save()) {
                        $result = 1;
                    } else {
                        $result = 0;
